...
def build():
    ...
    # make
    if repo_type == 'upstream' and ver_cmp(repo_ver, '5.0') < 0:
        make = dir_share_linux_tool + '/make-3.81'
    else:
        make = 'make'
...
def flash_image():
    ...
    if repo_type == 'stable':
        ...
        execute(path_share_fastboot + ' -s %s flash osloader %s/loader.efi' % (device_id, dir_workaround), interactive=True, dryrun=False)
        execute(path_share_fastboot + ' -s %s boot %s/fastboot.img' % (device_id, dir_workaround), interactive=True, dryrun=False)
