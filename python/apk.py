...
# This script is forked from https://github.com/yan12125/google-play-downloader, which is forked from https://github.com/evilsocket/google-play-downloader.
...
def parse_arg():
    ...
    parser = argparse.ArgumentParser(description='Download apk from Google Play',
...
''')
    ...
    parser.add_argument('--password', dest='password', help='Your android account password.')
    parser.add_argument('--package', dest='package', help='Package identifier')
    parser.add_argument('--device', dest='device', help='Your device ID')
    parser.add_argument('--proxy-host', dest='proxy_host', help='proxy host')
    parser.add_argument('--proxy-port', dest='proxy_port', help='proxy port')
    parser.add_argument('--dir-out', dest='dir_out', help='dir for download')

    parser.add_argument('--country', dest='country', help='Your country', default='USA')
    parser.add_argument('--operator', dest='operator', help='Your phone operator', default='AT&T')
    parser.add_argument('--sdklevel', type=int, dest='sdklevel', help='Android SDK API level', default=19)
    parser.add_argument('--device-name', dest='device_name', help='Device name', default='webperf')
...
def setup():
    ...
    if proxy_host and not proxy_port or not proxy_host and proxy_port:
        print('proxy host and port should both appear or not')


def download():
    print('Logging in ...')
    market = Market(args.email, args.password)
    market.login()

    print('Requesting package ...')
    operator = Operator(args.country, args.operator)
    request = AssetRequest(args.package, market.token, args.device, operator, args.device_name, args.sdklevel)
    (url, market_da) = market.get_asset(request.encode())

    print('Downloading...')
    Util.download_apk(args.package, url, market_da)
...
class Market:
    ...
    def login(self):
        ...
        if proxy_host:
            connection = httplib_compat.HTTPSConnection(proxy_host, proxy_port)
            connection.set_tunnel(Market.LOGIN_HOST, 443)
        else:
    ...
    def get_asset(self, request):
        ...
        if proxy_host:
            connection = httplib_compat.HTTPSConnection('child-prc.intel.com', 913)
            connection.set_tunnel(Market.API_HOST, 443)
        else:
...
class Util:
    ...
    def download_apk(package, url, market_da):
        filename = '%s-%s.apk' % (Util.get_datetime(), package)
