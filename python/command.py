...
commands = [
    ...
    ('basic', 'tar czf - chromium | ssh gyagp@ubuntu-ygu5-02 "cd /workspace/project; tar xzf -"', 'tar and copy to another machine', TYPE_NULL),
    ('basic', 'tar czf - chromium | ssh gyagp@ubuntu-ygu5-02 "cat >/tmp/chromium.tar.gz"', 'tar and copy to another machine', TYPE_NULL),
    ...
    ('basic', 'gigolo', 'visit shared folder in Windows from Linux', TYPE_RUNNABLE),
    ...
    ('basic', "sudo date -s '09/24/14 15:55'", 'set sys clock', TYPE_NULL),
    ...
    ('basic', "gconftool-2 --set --type boolean /desktop/gnome/peripherals/touchpad/touchpad_enabled true", 'enable synaptics', TYPE_RUNNABLE),
    ('basic', "sudo apt-key adv --recv-keys --keyserver keyserver.ubuntu.com $1", 'import key', TYPE_NULL),
    ('basic', "sudo ifconfig wlan0 down && sudo ifconfig wlan0 hw ether 02:87:f8:f6:e6:e6 && sudo ifconfig wlan0 up && sudo /etc/init.d/networking restart", 'change mac address', TYPE_NULL),
    ('basic', "rdesktop -f shwde9250.ccr.corp.intel.com -u ygu5 -d ccr -p xxx", 'rdesktop', TYPE_NULL),
    ('basic', "rsync -avsL /media/A3A9-6F07/* /workspace/backup/usb", 'notify', TYPE_NULL),
    ('basic', "eval $* && notify-send 'Cool~ \"$*\" finished!' -i 'emblem-cool' || notify-send 'Oh No! \"$*\" failed!' -i 'emblem-ohno'", 'rsync usb key', TYPE_NULL),
    ...
    ('network', 'sudo tcpdump -i eth0', 'sniff eth0', TYPE_RUNNABLE),
